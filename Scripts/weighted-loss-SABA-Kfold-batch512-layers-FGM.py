...
my_dataloader = tud.DataLoader(my_dataset,batch_size=512,shuffle=True,collate_fn=CustomDataset.collate_fn)
...
#         print(type_embed.shape,all_type_embedded.shape)
#         weighted_type_embed = F.softmax((type_embed @ all_type_embedded.transpose(0,1) / math.sqrt(Args.h_dim)),dim=-1) @  all_type_embedded
#         print(type_embed.shape,weighted_type_embed.shape)
...
#         print(merged_message.shape,type_embed.shape)
...
#             loss_weight = torch.masked_fill(torch.ones_like(batch_type),batch_type == 0,2).to(Args.device)
#             print(train_loss * loss_weight)
#             print(train_loss,loss_weight)
...
#                 print(all_pred_positive)
            ...
            if auc > self.best_auc[self.fold_idx] and mode=="Eval":
                ...
                print("第{}折最优AUC下的结果：auc:{},acc:{},precision:{},recall_sn:{},sp:{},f1:{},mcc:{},对应轮数为：{}轮".                      format(self.fold_idx,auc,acc,precision,recall_sn,sp,f1,mcc,epoch))
                plt.plot(fpr,tpr)
                plt.ylabel('True Positive Rate')
                plt.xlabel('False Positive Rate')
                plt.show()
#                 """存储最优模型"""
#                 torch.save(self.model.state_dict(),"../best_model/实验1/"+"fold_"+str(self.fold_idx)+".pkl")
    ...
    def train_epochs(self,Data):
        ...
        self.fold_idx = 0
        for train_index,eval_index in skf.split(Data.Train.seqs,Data.Train.labels):
            self.writer = SummaryWriter("../tensorboard/实验6(A40)/batch/6.6_w_loss_FGM_512_layer_{}_fold-".format(Args.num_layers)+str(self.fold_idx))
            self.model = Blocks().to(Args.device)
            self.optimizer = optim.AdamW(self.model.parameters(),lr=2e-4)
#             print(train_seqs)
#             print(train_index)
            train_dataset = CustomDataset(Data.Train.seqs[train_index],Data.Train.labels[train_index],Data.Train.usage[train_index],Data.Train.name[train_index])
            eval_dataset = CustomDataset(Data.Train.seqs[eval_index],Data.Train.labels[eval_index],Data.Train.usage[eval_index],Data.Train.name[eval_index])
            train_dataloader = tud.DataLoader(train_dataset,batch_size=Args.batch_size,shuffle=True,collate_fn=CustomDataset.collate_fn)
            eval_dataloader = tud.DataLoader(eval_dataset,batch_size=Args.batch_size,shuffle=True,collate_fn=CustomDataset.collate_fn)
...
#         print(TP,FP,TN,FN)
        ...
        mcc = (TP * TN - FP * FN) / np.sqrt(((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN)) + alpha)
