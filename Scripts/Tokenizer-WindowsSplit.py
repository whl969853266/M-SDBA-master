...
class Tokenizer():
    ...
    def get_files_seq_token(pth:str, data_col_name:str, label_col_name:str, fit=False):
        ...
        return res,list(df[data_col_name].apply(lambda x:x.strip())),torch.tensor(df[label_col_name])
...
tokens,all_seqs,all_labels = tokenizer.get_files_seq_token("/home/zf/DNA N4/Datasets_pro-merge.csv","data","label")
...
torch.save([tokenizer.rna_dict,encode_res[1],all_labels,torch.LongTensor(df["usage_idx"]),torch.LongTensor(df["name_idx"])],"Datasets_pro-merge_w1s1.pkl")
